...
def add_test_page(page_name: str, used_templates: tuple = ('base.html',), entity_attr: str | None = None) -> Callable:
    def decorator(cls) -> type:
        def test_page(self):
            response = self.client.get(reverse(page_name) if not(
                entity_attr) else reverse(page_name, args=[getattr(self, entity_attr).id]))
...
class TestPage(TestCase):
    def setUp(self):
        ...
        self.tournament = Tournament.objects.create(
            title='tournament', owner=self.user, start='2021-01-01', end='2021-01-02')
        ...
        self.match = Match.objects.create(tournament=self.tournament, team1=self.team1,
                                          team2=self.team2, place=self.place, owner=self.user, match_date_time=date_time)
