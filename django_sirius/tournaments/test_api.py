...
from tournaments.models import Match, Place, Team, Tournament
...
def add_test_created(model, path: str, creation_attrs: dict) -> Callable:
    def decorator(cls) -> type:
        def test_created(self):
            ...
            self.assertIn(
                response.data['id'],
                [str(obj.id) for obj in model.objects.all()]
            )
...
class TeamTestCase(SetUpMixin):
    def test_future_founding_date(self):
        response = self.client.post(
            ...
            {'title': 'team1', 'founding': '3000-01-01'}
        )
    ...
    def test_past_founding_date(self):
        response = self.client.post(
            reverse('api:team-list'),
            {'title': 'team1', 'founding': '2000-01-01'}
        )
...
@add_permission_test(
    'api:tournament-list',
    {'title': 'tournament1', 'start': '2021-01-01', 'end': '2022-01-01'}
)
@add_test_created(
    Tournament,
    'api:tournament-list',
...
@add_null_field_test(
    'api:tournament-list',
    'title',
    {'title': '', 'start': '2021-01-01', 'end': '2022-01-01'}
)
@add_null_field_test(
    'api:tournament-list',
    'start',
    {'title': 'tournament1', 'start': '', 'end': '2022-01-01'}
)
@add_null_field_test(
    'api:tournament-list',
    'end',
    {'title': 'tournament1', 'start': '2021-01-01', 'end': ''}
)
class TournamentTestCase(SetUpMixin):
    def test_start_later_then_end(self):
        with self.assertRaises(IntegrityError):
            self.client.post(
                reverse('api:tournament-list'),
                {
                    'title': 'tournament1',
                    'start': '2022-01-01',
                    'end': '2021-01-01'
...
class MatchTestCase(SetUpMixin):
    def setUp(self):
        ...
        self.team2_id = self.client.post(
            reverse('api:team-list'),
            {'title': 'team2', 'founding': '2000-01-01'}
        ).data['id']
        self.place_id = self.client.post(
            reverse('api:place-list'),
            {'title': 'place', 'address': 'city'}
        ).data['id']
        self.tournament_id = self.client.post(
            reverse('api:tournament-list'),
            {'title': 'tournament', 'start': '2020-01-01', 'end': '2020-01-10'}
        ).data['id']
